<?php
	...
	$encryption_key		= 'SecretGNU';			// Unique 8 to 32 character encryption key
	$notification_email	= 'jacobbarkdull@gmail.com';	// E-mail for notification of new comments
	$admin_nickname		= 'Jacob Barkdull';		// Nickname with admin rights (must be title-cased)
	$admin_password		= '#@sh0ver2014ftw!?%^';	// Password to gain admin rights
	...
	if (isset($_GET['source']) and basename($_SERVER['PHP_SELF']) == basename(__FILE__)) {
		...
		echo "\t" . '// Copyright (C) 2013 Jacob Barkdull' . PHP_EOL;
		echo "\t" . '//' . PHP_EOL;
